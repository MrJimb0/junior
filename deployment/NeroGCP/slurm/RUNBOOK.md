# Nero corpus-build runbook (build on Nero → consume on the laptop)

The plan: do the heavy GPU work — reading the charts and turning every text chunk into
an embedding (a numeric vector capturing its meaning) — on the Nero cluster,
then copy the finished, searchable "corpus" back to the laptop where you actually
develop recipes and pull values out of the charts. "Nero" is <YOUR_INSTITUTION>'s research
compute cluster; the laptop is your everyday machine.

Authoritative ordered steps. Supersedes any loose commands kept outside the repo.
Verified locally 2026-06-20 (the embedding model works the same on both machines, and a full
per-step seal → ingest → embed → index → extract round-trip ran with the Nero
`float16`/`safetensors` config). Nero-side steps still require the <YOUR_INSTITUTION> VPN + a cluster account.

## Path facts (why the split works)
- `output_root` (cfg) binds `JR_DATA_ROOT`, so Nero (`JR_OUT_ROOT`) and the laptop produce the
...
- The two machines agree they are using the same embedding model by comparing **content hashes**
  ...
  deliberately NOT compared, so the model can sit at a different path on Nero than on the laptop.
  The laptop `model.safetensors` already hashes `ea7388…`, matching the copy pinned on Nero.

## One-time setup on Nero
...
- Push inputs to Nero: `bash …/rclone_sync.sh pull` (Box→`LOCAL_RAW`) or `sftp <YOUR_TRANSFER_HOST>`.
...
### 2. Build the corpus on Nero (login node, then submit cluster jobs with sbatch)
...
SLURM=deployment/NeroGCP/slurm
...
PYTHONPATH=src:. .venv-arm64/bin/python $PWD/deployment/NeroGCP/slurm/check_encoder_alignment.py \
...
## Gotchas
- `seal` is mandatory before every per-step run (Nero and laptop); editing a recipe needs a re-seal or a new run_id.
