...
#   cd "$JR_CODE_ROOT" && mkdir -p logs && sbatch --export=ALL,CFG=deployment/NeroGCP/slurm/index_override.yaml,PATIENT_LIST=<YOUR_CODE_ROOT>/patient_ids.txt \
#          --array=1-500%20 deployment/NeroGCP/slurm/index.sh
...
CFG="${CFG:?must set CFG=deployment/NeroGCP/slurm/index_override.yaml}"
