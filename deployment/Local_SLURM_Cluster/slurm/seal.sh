#!/usr/bin/env bash
# seal.sh — run ONCE on the cluster LOGIN NODE before submitting any jobs (sbatch).
#
...
#   CFG=deployment/Local_SLURM_Cluster/slurm/ingest_override.yaml \
#   PATIENT_LIST=<YOUR_CODE_ROOT>/patient_ids.txt \
#   bash deployment/Local_SLURM_Cluster/slurm/seal.sh
#
