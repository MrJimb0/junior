#!/usr/bin/env bash
# rclone_sync.sh — move patient data between your secure Box tenancy and the cluster.
# (rclone is a file-copy tool; "remote" = a named cloud/Box location it can read/write.)
# The cluster is <YOUR_INSTITUTION>'s secure compute cluster. "pull" brings raw charts
# down to it before a run; "push" sends a finished run's outputs back to Box.
#
...
#
# Usage:
#   bash deployment/Local_SLURM_Cluster/slurm/rclone_sync.sh pull
#   bash deployment/Local_SLURM_Cluster/slurm/rclone_sync.sh push <run_id>
#
...
#   * "ml rclone" loads rclone from the cluster software-module system; rclone is
...
# Ensure rclone is on PATH (module-loaded on the cluster).
...
case "${1:-}" in
  pull)
    ...
    ;;
  push)
    RUN_ID="${2:?push requires a run_id: bash deployment/Local_SLURM_Cluster/slurm/rclone_sync.sh push <run_id>}"
